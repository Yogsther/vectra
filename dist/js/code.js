...
function updateBookingPosition(x) {
	if (x) {
		...
		if (proposedBookingPosition < lowestAlloedValue) proposedBookingPosition = lowestAlloedValue;
...
document.getElementById("booking-canvas").addEventListener("touchmove", function (e) {
	e.preventDefault()
...
// toucn end
document.getElementById("booking-canvas").addEventListener("touchend", function (e) {
	e.preventDefault()
	confirmBookingPosition()
})

// Add touch support to mousedownovercanvas
document.getElementById("booking-canvas").addEventListener("touchstart", function (e) {
	e.preventDefault()
	var touch = e.touches[0]
	var x = touch.clientX
	var rect = this.getBoundingClientRect()
...
function renderCanvas() {
	...
	while (true) {
		...
		let day = date.getDay()
		let dayString = day == 0 ? "Söndag" : day == 1 ? "Måndag" : day == 2 ? "Tisdag" : day == 3 ? "Onsdag" : day == 4 ? "Torsdag" : day == 5 ? "Fredag" : "Lördag"
...
function updateReturnTime() {
	...
	if (info.booked) {
		...
		setStatusText("Booked from " + info.clock + " to " + bookingTime + " by " + info.name, "Back in about " + timeString)
	} else {
